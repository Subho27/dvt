...
result_hm_filenames = [
    f"./animation/heatmap_plots/{name.lower().replace('-', '_').replace(' ', '_')}_animation.gif" 
...
for index, nc_file in enumerate(nc_files):
    ...
    vmin, vmax = nc_df[nc_data[index]].min(), nc_df[nc_data[index]].max()
    ...
    for date in dates:
        ...
        # Create a 2D grid for heatmap plot
        ...
        # Plot heatmap for the specific date
        plt.figure(figsize=(10, 6))
        plt.imshow(pivot_values, origin='lower', extent=[lon.min(), lon.max(), lat.min(), lat.max()],
                   vmin=vmin, vmax=vmax, cmap='inferno')
        plt.colorbar(label=label_names[index])
        plt.title(f"{label_names[index]} Heatmap - {date}")
        plt.xlabel("Longitude")
        plt.ylabel("Latitude")
    
        # Save plot as an image file
        file_name = f"heatmap_{date}.png"
        plt.savefig(f"./dumps/{file_name}")
        image_files_heatmap.append(file_name)  # Add file to the list of image files
        plt.close()
    
    # Generate GIF using imageio
    with imageio.get_writer(result_hm_filenames[index], mode='I', duration=2.5) as writer:
        for file_name in image_files_heatmap:
